...
(function() {
  var LANG_KEY = 'expatcalc_lang';
  window.saveLangPref = function(lang) {
    try { localStorage.setItem(LANG_KEY, lang); } catch(e) {}
  };
  window.loadLangPref = function() {
    try { return localStorage.getItem(LANG_KEY) || 'en'; } catch(e) { return 'en'; }
  };
  // Auto-apply saved language on DOMContentLoaded
  // Each page should define its own SUPPORTED_LANGS array and setLang function
  // and optionally window.PAGE_DEFAULT_LANG (defaults to 'en')
  window.addEventListener('DOMContentLoaded', function() {
    var saved = window.loadLangPref();
    var pageDef = window.PAGE_DEFAULT_LANG || 'en';
    if (saved && saved !== pageDef && typeof window.setLang === 'function' && window.SUPPORTED_LANGS) {
      // Only apply if this page supports the saved language
      if (window.SUPPORTED_LANGS.indexOf(saved) !== -1) {
        var btn = document.querySelector('.lang-btn[data-lang="' + saved + '"]');
        window.setLang(saved, btn);
      }
    }
  });
})();
...
(function() {
  function setupScrubber(canvasId, computeFn, formatFn) {
    ...
    canvas.addEventListener('mousemove', function(e) {
      ...
      rafId = requestAnimationFrame(function() {
        ...
        var pct = relX / plotW;
        var minG = parseFloat(canvas.dataset.minGross) || 500;
        var maxG = parseFloat(canvas.dataset.maxGross) || 12000;
  ...
  window.addEventListener('load', function() {
    setTimeout(function() {
      ...
      // Language-aware translation helper (reads window.lang set by each calculator page)
      var tS = function(de, en, hu, nl) {
        var l = window.lang || 'en';
        if (l === 'de') return de;
        if (l === 'hu') return hu || en;
        if (l === 'nl') return nl || en;
        return en;
      };

      // Net vs Gross chart
      setupScrubber('netVsGrossChart', function(g) {
        return computeFn(g, true);
      }, function(g, d) {
        return '<div class="tt-row"><div class="tt-dot" style="background:#3b82f6"></div><span class="tt-label">' + tS('Brutto','Gross','Bruttó','Bruto') + ':</span><span class="tt-value">' + fmt2(g) + ' ' + currSym + '</span></div>' +
               '<div class="tt-row"><div class="tt-dot" style="background:#2ecc71"></div><span class="tt-label">' + tS('Netto','Net','Nettó','Netto') + ':</span><span class="tt-value">' + fmt2(d.net) + ' ' + currSym + '</span></div>' +
               '<div class="tt-row"><div class="tt-dot" style="background:#e94560"></div><span class="tt-label">' + tS('Abzüge','Deductions','Levonások','Inhoudingen') + ':</span><span class="tt-value">' + fmt2(d.deductions) + ' ' + currSym + '</span></div>';
      });

      // Tax rate chart
      setupScrubber('taxRateChart', function(g) {
        var w = computeFn(g, true);
        var effRate = g > 0 ? (w.deductions / g * 100) : 0;
        var delta = 100;
        var wPlus = computeFn(g + delta, true);
        var margRate = ((wPlus.deductions - w.deductions) / delta) * 100;
        return { effRate: effRate, margRate: Math.max(0, margRate) };
      }, function(g, d) {
        return '<div class="tt-row"><div class="tt-dot" style="background:#3b82f6"></div><span class="tt-label">' + tS('Brutto','Gross','Bruttó','Bruto') + ':</span><span class="tt-value">' + fmt2(g) + ' ' + currSym + '</span></div>' +
               '<div class="tt-row"><div class="tt-dot" style="background:#f1c40f"></div><span class="tt-label">' + tS('Effektiv','Effective','Effektív','Effectief') + ':</span><span class="tt-value">' + d.effRate.toFixed(1) + '%</span></div>' +
               '<div class="tt-row"><div class="tt-dot" style="background:#E87722"></div><span class="tt-label">' + tS('Grenzrate','Marginal','Marginális','Marginaal') + ':</span><span class="tt-value">' + d.margRate.toFixed(1) + '%</span></div>';
      });
    }, 500);
  });
})();
